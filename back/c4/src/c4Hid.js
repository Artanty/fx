...
function findC4() {
  ...
  return (
    list.find(d => d.productId === AS_PID_C4) ||
...
// unless we are certain we have the C4 Synth (matching PID, or the known
// usage-page/interface), so we never send commands to the wrong HID device.
function requireC4() {
  const d = findC4();
  if (!d) throw new Error('Source Audio C4 Synth (VID 0x29a4 PID 0x0302) not found — check the USB connection.');
  const ok = d.productId === AS_PID_C4 || (d.usagePage === 0xffa0 && d.interface === 2);
  if (!ok) {
    throw new Error(
      `Found a Source Audio device but not the C4 Synth (PID 0x${d.productId ? d.productId.toString(16) : '?'}) — refusing to write.`
    );
  }
  return d;
}
